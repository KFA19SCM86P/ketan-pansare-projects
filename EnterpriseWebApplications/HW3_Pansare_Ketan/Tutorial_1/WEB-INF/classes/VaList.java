...
public class VaList extends HttpServlet {
	...
	/* Checks the Vas type whether it is microsft or apple or samsung */
		...
		{
			hm.putAll(MySqlDataStoreUtilities.selectVa());
			name = "";
		} 
		else 
		{
			...
			{	
				for(Map.Entry<String,Va> entry : MySqlDataStoreUtilities.selectVa().entrySet())
				{
		...
		for (Map.Entry<String, Va> entry : hm.entrySet()) {
			...
			pw.print("<li><form method='post' action='ViewReview'>"+"<input type='hidden' name='name' value='"+Va.getName()+"'>"+
					"<input type='hidden' name='type' value='vas'>"+
					"<input type='hidden' name='maker' value='"+CategoryName+"'>"+
					"<input type='hidden' name='access' value=''>"+
